...
namespace PixiJS
{
	...
	FCM::Result OutputWriter::EndDocument()
	{
		...
		meta.push_back(JSONNode("libsPath", m_libsPath));
...
//		ext = 'png'
		...
		if (ext != "png" && ext != "jpg" && ext != "jpeg")
    {
      ext = "png";
		...
		Utils::GetFileNameWithoutExtension(libPathName, name);
		SetImageExportFileName(libPathName, name);
//		Utils::Trace(m_pCallback, "INFO44: Output image folder (%s) (%s) %s  could not be created\n", libPathName.c_str(), name.c_str(), alreadyExported);
...
//			std::map<std::string, std::int>::iterator it = m_imageNameMap.find(name);
//			std::string name
...
//        return true;
//      }
//			Utils::Trace(m_pCallback, "INFO44: Output image folder (%s) (%s) could not be created\n", libPathName.c_str(), ext.c_str());
//			Utils::Trace(m_pCallback, "INFO3333: Output image folder (%s) could not be created\n", name.c_str());
...
/*	FCM::Boolean OutputWriter::GetImageExportName(const std::string& name)
	{
		std::map<std::string, std::string>::iterator it = m_imageNameMap.find(name);
...
//			name = it->second;
//			newName = name + '~';
//			m_imageNameMap[name] = newName
//			name = newName
//			return true;
//		}
//		m_imageNameMap.insert(std::pair<std::string, std::string>(name, name))
		return false;
	}*/
	FCM::Boolean OutputWriter::GetImageExportName(std::string& libPathName, std::string& name)
	{
		std::map<std::string, std::string>::iterator it = m_imageNameMap.find(libPathName);
		...
		if (it != m_imageNameMap.end())
		{
			// Image already exported
			count = it->second;
			std::istringstream is(count); //构造输入字符串流，流的内容初始化为“12”的字符串
      int i;
      is >> i;
//			char * a=(char *) name.c_str();
//atoi(name.c_str());
//      int value = atoi(const char* a);
			 i ++;
////						newName = name + '~';
      			m_imageNameMap[libPathName] = Utils::ToString(i);
      			name = libPathName + Utils::ToString(i);
			return true;
		}
m_imageNameMap[libPathName] = count;
		return false;
	}
